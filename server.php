...
class WebSocket {
    ...
    private function doServer() {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                list($msg,$touname) = self::dealMsg($socket, $recv_msg);

                $this->broadcast($msg,$touname);
            }
        }
    }
    ...
    private function dealMsg($socket, $recv_msg) {
        ...
        if(preg_match('/^@(\w+)\s(.+)$/',$recv_msg['content'],$tmp_msg_content)){
            ...
            array_push($touname,$this->sockets[(int)$socket]['uname'],$tmp_msg_content[1]);
        }else{
        ...
        return array($this->build(json_encode($response)),array_unique($touname));
    }
     ...
     * @param $data
     */
    private function broadcast($data,$touname) {
        foreach ($this->sockets as $socket) {
            if ($socket['resource'] == $this->master) {
                continue;
            }
            if($touname){
                if(in_array($socket['uname'],$touname) ){
...
$ws = new WebSocket("192.168.86.128", "5555");